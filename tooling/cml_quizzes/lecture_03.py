QUIZ_L03 = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "explanation": "R^2 (the Coefficient of Determination) measures the proportion of total variance in the dependent response variable Y that is statistically explained by the independent predictor features in the model: R^2 = 1 - (SS_{res} / SS_{tot}).",
      "difficulty": "easy",
      "subtopic": "R^2 & Metric Calculations"
    },
    {
    ...
    },
    {
      ...
      "explanation": "Adding a predictor gives the least-squares optimization extra degrees of freedom, guaranteeing SS_{res} <= SS_{res,old}; therefore raw R^2 cannot decrease. However, if the new predictor explains less variance than the penalty term (n-1)/(n-p-1), Adjusted R^2 decreases.",
      "difficulty": "easy",
      "subtopic": "Adjusted R^2"
    },
    {
    ...
    },
    {
      ...
      "explanation": "Option A strictly implements the mathematical formulas: SSE = \\sum (y - \\hat{y})^2, SST = \\sum (y - \\bar{y})^2, R^2 = 1 - (SSE/SST), and R^2_{adj} = 1 - (1 - R^2)(n-1)/(n-p-1).",
      "difficulty": "medium",
    ...
    },
    {
      ...
      "options": [
        {
          "id": "A",
          "text": "Setting all slope parameters \\theta_1, ..., \\theta_d to zero."
        },
        {
          "id": "B",
      ...
      "explanation": "By defining x_0 = 1 for every observation, the augmented feature vector \\mathbf{x} = [1, x_1, ..., x_d]^T and parameter vector \\boldsymbol{\\theta} = [\\theta_0, \\theta_1, ..., \\theta_d]^T combine into h(\\mathbf{x}) = \\boldsymbol{\\theta}^T \\mathbf{x}.",
      "difficulty": "easy",
    ...
    },
    {
      "id": "cml03-q14",
      "type": "single_choice",
      "question": "Assuming the Gram matrix X^T X is non-singular (full column rank), what is the celebrated analytical closed-form solution known as the Normal Equation?",
      "options": [
        {
          "id": "A",
          "text": "\\boldsymbol{\\theta}^* = X (X^T X)^{-1} \\mathbf{y}"
        },
        {
          "id": "B",
          "text": "\\boldsymbol{\\theta}^* = (X^T X)^{-1} X^T \\mathbf{y}"
        },
        {
          "id": "C",
          "text": "\\boldsymbol{\\theta}^* = (X X^T)^{-1} X \\mathbf{y}"
        },
        {
          "id": "D",
          "text": "\\boldsymbol{\\theta}^* = X^T (X X^T)^{-1} \\mathbf{y}"
        }
      ],
      "correctOptionIds": [
        "B"
      ],
      "explanation": "Setting the matrix gradient \\nabla_{\\boldsymbol{\\theta}} J = X^T X \\boldsymbol{\\theta} - X^T \\mathbf{y} = \\mathbf{0} yields X^T X \\boldsymbol{\\theta} = X^T \\mathbf{y}. Multiplying by (X^T X)^{-1} gives \\boldsymbol{\\theta}^* = (X^T X)^{-1} X^T \\mathbf{y}.",
      "difficulty": "medium",
    ...
    },
    {
      "id": "cml03-q16",
      "type": "single_choice",
      "question": "What is the Hat Matrix H = X(X^T X)^{-1} X^T, and what is its mathematical trace \\text{Tr}(H)?",
      "options": [
        {
          "id": "A",
      ...
      "explanation": "H is symmetric (H = H^T) and idempotent (H^2 = H). Using the cyclic property of the trace: \\text{Tr}(H) = \\text{Tr}(X(X^T X)^{-1} X^T) = \\text{Tr}((X^T X)^{-1} (X^T X)) = \\text{Tr}(I_{d+1}) = d + 1 (the number of fitted parameters).",
      "difficulty": "hard",
      "subtopic": "Hat Matrix"
    },
    {
      "id": "cml03-q17",
      "type": "single_choice",
      "question": "What is the computational time complexity of solving the Normal Equation \\boldsymbol{\\theta}^* = (X^T X)^{-1} X^T \\mathbf{y} for a dataset with n instances and d features?",
      "options": [
        {
          "id": "A",
      ...
      "explanation": "Computing the Gram matrix X^T X requires \\mathcal{O}(n d^2) operations. Inverting the (d+1) \\times (d+1) Gram matrix requires \\mathcal{O}(d^3) operations. Total complexity is \\mathcal{O}(n d^2 + d^3).",
      "difficulty": "medium",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "id": "B",
          "text": "Yes, on a held-out test set if the model makes worse predictions than the naive baseline model \\hat{y} = \\bar{y}_{train}, causing SS_{res} > SS_{tot}."
        },
        {
          "id": "C",
      ...
      "explanation": "On a test set, R^2 = 1 - (SS_{res} / SS_{tot}). If a model performs worse than a simple horizontal line predicting the mean of the training target, SS_{res} exceeds SS_{tot}, driving R^2 below zero (indicating catastrophic failure).",
      "difficulty": "medium",
      "subtopic": "R^2 & Metric Calculations"
    },
    {
    ...
    },
    {
      "id": "cml03-q22",
      "type": "single_choice",
      "question": "What happens to the Gram matrix X^T X when the number of features d exceeds the number of training observations n (d > n)?",
      "options": [
        {
          "id": "A",
          "text": "X^T X becomes an identity matrix."
        },
        {
          "id": "B",
          "text": "X^T X is strictly non-invertible (singular) because its rank is at most n < d+1, yielding infinitely many solutions that achieve zero training error."
        },
        {
          "id": "C",
          "text": "X^T X has a determinant of +\\infty."
        },
        {
          "id": "D",
      ...
      "explanation": "The rank of X \\in \\mathbb{R}^{n \\times (d+1)} is at most n. The Gram matrix X^T X is (d+1) \\times (d+1). If n < d+1, X^T X has rank at most n < d+1, making it rank-deficient and singular (det = 0). Regularization (Ridge) is required to make it invertible.",
      "difficulty": "hard",
      "subtopic": "Normal Equation"
    },
    {
    ...
    },
    {
      "id": "cml03-q24",
      "type": "multi_choice",
      "question": "Which of the following conditions cause the Gram matrix X^T X to become non-invertible (singular)? (Select ALL that apply)",
      "options": [
        {
          "id": "A",
      ...
      "explanation": "A, B, and C cause rank-deficiency in X, rendering X^T X singular. D (standardization) maintains matrix rank and often improves numerical stability.",
      "difficulty": "medium",
      "subtopic": "Normal Equation"
    },
    {
      "id": "cml03-q25",
      "type": "multi_choice",
      "question": "Which of the following statements regarding the Hat Matrix H = X(X^T X)^{-1} X^T are mathematically TRUE? (Select ALL that apply)",
      "options": [
        {
          "id": "A",
